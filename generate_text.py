...
def sample_sampling_strategy(sequence_length, model, output, hidden, vocab, temperature=0.5):
    ...
    for ii in range(sequence_length):
        ...
        outputs.append(sample)
        output, hidden = model.inference(sample, hidden, 0.5)
    return outputs
...
def generate_language(model, vocab, seed_words, sequence_length,  device="cpu", sampling_strategy='max', beam_width=10, temperature=0.5):
    model.eval()
    with torch.no_grad():
